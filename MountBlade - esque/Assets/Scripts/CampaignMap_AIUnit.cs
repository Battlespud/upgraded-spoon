...
public class CampaignMap_AIUnit : MonoBehaviour {

    public CampainMap_POI cmPOI; //this is public so that we can access it without having to use GetComponent everytime
    ...
    void AttackTarget()
    {
        ...
        if (distanceToTarget < 5)
        {
            ...
            {                           
                overrideAIstate = false;
                hasDestination = false;
                enemyToAttack.GetComponent<CampaignMap_AIUnit>().aiState = AiState_Unit_Campaign.Retreat;
                ...
                //{
                    ...
                    if (cmPOI.cmManager.FactionList[cmPOI.FactionNumber].EnemiesWith.Contains(_cmPOI.FactionNumber))
                    {
                        ...
                        if (_cmPOI.Point_of_Interest_Type == CampainMap_POI.POItype.Unit)
                        {
                            ...
                            if (_cmPOI.transform.GetComponent<CampaignMap_AIUnit>().aiState != AiState_Unit_Campaign.Retreat)
                            {
                                //Override AI state and attack the target
                                GiveTargetToAttack(_cmPOI);
                                //and tell the enemy that we are attacking
                                _cmPOI.transform.GetComponent<CampaignMap_AIUnit>().GiveTargetToAttack(cmPOI);
                            }
                            else
                            {
                                overrideAIstate = false;
